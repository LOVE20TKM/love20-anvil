...
export { anvilAccounts } from './anvil.mjs';
...
export function collectGroupActionSeedAddresses(graph, root = repoRoot) {
  ...
  const addresses = {
    ...
    groupVerifyAddress: extensionGroup.groupVerifyAddress,
    groupActionFactoryAddress: extensionGroup.groupActionFactoryAddress,
    groupRecipientsAddress: extensionGroup.groupRecipientsAddress,
  ...
  if (missing.length > 0) {
    throw new Error(`Missing group action seed address(es): ${missing.join(', ')}`);
  }
...
export function collectGroupChatSeedAddresses(graph, root = repoRoot) {
  ...
  const addresses = {
    ...collectGroupActionSeedAddresses(graph, root),
...
function ensureFirstTokenBalances(runner, addresses, accounts) {
  ...
  claimFirstTokenIfPossible(runner, addresses, [anvilAccounts[0], ...targetAccounts]);
...
export function prepareGroupActionRound(graph, options = {}) {
  const root = options.root || repoRoot;
  const runner = options.runner || new CastRunner({
    rpcUrl: graph.network.rpcUrl,
    root,
  ...
  const state = buildGroupChatSeedState({
    addresses,
    anvil: readAnvilMetadata(runner),
  });
  ...
  return { addresses, joinSetup, roles, runner, setup, state };
}
